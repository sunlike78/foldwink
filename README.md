...
- **Content:** **Current library: 98 curated puzzles** (65 easy + 33 medium). Every medium carries the Foldwink Tabs mechanic via per-group `revealHint` keywords. **Target library: 500 curated puzzles** via the disciplined batch pipeline in `docs/content/BATCH_WORKFLOW.md`. 500 does not exist yet and will not be claimed anywhere in live docs until it does.
